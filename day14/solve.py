...
def normalize_coords(lines, origin):
    for line in lines:
        for coord in line:
            coord[0] = origin - (500 - coord[0])
...
if __name__ == "__main__":
    ...
    for line in lines:
        for coord in line:
            left_bound = min(left_bound, coord[0])
            right_bound = max(right_bound, coord[0])
            depth_bound = max(depth_bound, coord[1])


    grid = [['.' for _ in range(right_bound - left_bound + 1)] for _ in range(depth_bound + 1)]
    origin = 500 - left_bound
    left_bound = 0
    right_bound = right_bound - 500 + origin

    normalize_coords(lines, origin)
